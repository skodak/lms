...
class cache implements cache_loader {
    ...
    /**
     * Add prefix to normal cache key to get external locking key.
     *
     * @param $key
     * @return string
     */
    protected function get_lock_key($key): string {
        return 'cache_lock_' . $key;
    }

    /**
     * Checks if this cache has an external lock on the given key.
     *
     * @param string|int $key
     * @return bool
     */
    public function check_lock_state($key): bool {
        $lockkey = $this->get_lock_key($key);

        $lock = $this->store->get($lockkey);
        if ($lock === false) {
            return false;
        }
        return (time() - $lock < $this->definition->get_lockexpiry());
    }

    /**
     * Acquires an external lock on the given key.
     *
     * @param string|int $key The key as given to get|set|delete
     * @return true
     * @throws \moodle_exception when lock request times out
     */
    public function acquire_lock($key) {
        ...
        while (true) {
            ...
            if (time() - $start > $this->definition->get_lockexpiry()) {
                throw new moodle_exception('ex_locktimeout', 'core_cache');
            }
            sleep(1);
        }
        $lockkey = $this->get_lock_key($key);
        $this->store->set($lockkey, time());
        return true;
    }

    /**
     * Releases external lock this cache has on the given key.
     *
     * @param string|int $key
     * @return true
     */
 ...
 *
 * NOTE: this class is internal, don't use me directly.
 *
 * @package    core
 * @category   cache
 * @copyright  2012 Sam Hemelryk
